...
server <- function(input, output) {
  ...
  contents1 <- reactive({
    ...
    predictions <- cbind.data.frame(`Sample` = rownames(df_sg),`Prediction (%)`= predict(model, df_sg))
    print(predictions)
  }
)
...
ui <- fluidPage(
  ...
  # Navigation bar
  navbarPage("Quantification of Petroleum Wax Blends"),
  
  # Input: File type and preprocessing instructions 
  helpText(style = "text-align: justify;", "This Shiny App allows percentage predictions of 
           paraffin and micro-wax blends. The predictions are performed using a predictive model based on Boruta-PLS algorithm."),
  
  helpText(style = "text-align: justify;", "Note: A total microwax content would be 0% and a paraffin wax content would be 100%."),
 ...
 dashboardHeader(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      #Output: Text
      tags$label(h5('• The predictions expressed as blend percentage are:')),
